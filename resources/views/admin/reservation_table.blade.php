...
<a href="{{ route('reservation_trashshow')}}" class="btn btn-danger" style="margin-left:20px;font-size: 14px; padding: 8px 16px; border-radius: 4px; text-decoration: none; color: #fff; background-color: rgb(255, 0, 64); border: 1px solid #ff0080;width:100px;height:40px;">Trash Bin</a>
...
<table class="table table-hover my-0">
    ...
    @endphp --}}
        ...
        <tr>
            ...
            <td style="color:red;">{{ $usrdata->reservation_status }}</td>
            <td>{{ $usrdata->created_at }}</td>
            <td>
                <a href="{{ route('reservation_delete',['id' => $usrdata->id]) }}" class="btn btn-danger">Trash</a>
                <br><br><a href="{{ route('done',['id' => $usrdata->id]) }}" class="btn btn-info">Done</a>
            </td>
            <td>
